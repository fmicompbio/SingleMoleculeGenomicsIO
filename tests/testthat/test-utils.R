...
## -------------------------------------------------------------------------- ##
## Checks, countAlignmentsInBam
## -------------------------------------------------------------------------- ##
test_that("countAlignmentsInBam works", {
    bamfiles <- system.file("extdata", c("6mA_1_10reads.bam",
                                         "6mA_2_10reads.bam"),
                            package = "SingleMoleculeGenomicsIO")
    bamfileNoheader <- tempfile(fileext = ".bam")

    expect_error(countAlignmentsInBam(),
                 'argument "bamfile" is missing, with no default')
    expect_error(countAlignmentsInBam(1L),
                 "Expecting a single string value")
    expect_error(countAlignmentsInBam(bamfiles),
                 "Expecting a single string value")
    expect_error(countAlignmentsInBam(bamfileNoheader),
                 "Could not open input file")
    # ... no bam index
    tmpbam <- tempfile(fileext = ".bam")
    expect_true(file.copy(from = bamfiles[1], to = tmpbam))
    expect_error(countAlignmentsInBam(tmpbam),
    ...
    res <- countAlignmentsInBam(bamfiles[1])
    expect_type(res, "list")
    expect_named(res, c("mapped", "unmapped"))
    expect_named(res$mapped, expected_chrs)
    expect_identical(res$mapped[["chr1"]], 10)
    expect_identical(sum(res$mapped), 10)
    expect_identical(res$unmapped, 0)
})
