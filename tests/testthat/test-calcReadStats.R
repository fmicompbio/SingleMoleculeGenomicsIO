    ...
    resL <- lapply(allReadStats, function(param) {
        ...
        if (param %in% c("ACModProb", "PACModProb")) {
            ...
            expect_equal(
                ...
                switch(param,
                       "MeanModProb" = unname(
                           SparseArray::colMeans(mat[, useReads],
                                                 na.rm = TRUE)),
                       "FracMod" = unname(
                           SparseArray::colMeans(mat[, useReads] > 0.5,
                                                 na.rm = TRUE)),
                       "MeanConf" = unname(
                           SparseArray::colMeans(.callConf(mat[, useReads]),
                                                 na.rm = TRUE)),
                       "MeanConfUnm" = unname(
                           SparseArray::colMeans(.callConf(matUnmod[, useReads]),
                                                 na.rm = TRUE)),
                       "MeanConfMod" = unname(
                           SparseArray::colMeans(.callConf(matMod[, useReads]),
                                                 na.rm = TRUE)),
                       "FracLowConf" = unname(
                           SparseArray::colMeans(abs(0.5 - mat[, useReads]) < 0.2,
                                                 na.rm = TRUE)),
